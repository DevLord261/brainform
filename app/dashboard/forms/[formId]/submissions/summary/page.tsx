...
const processSubmissions = (fields: FormField[], submissions: Submission[]) => {
  ...
  const questionCharts = fields
    .filter((field) => !["hidden", "password", "signature"].includes(field.type))
    .map((field) => {
      ...
      if (["select", "radio", "checkbox"].includes(field.type)) {
        ...
        return { type: "pie", questionLabel, data: chartData, totalResponses: responses.length }
      }

      if (["checkbox-group"].includes(field.type)) {
        const allChoices = responses.flatMap((r) => (Array.isArray(r) ? r : String(r).split(",")))
        ...
        return { type: "bar", questionLabel, data: chartData, totalResponses: responses.length }
      }
      ...
      if (field.type === "rating") {
        ...
        const chartData = Array.from({ length: maxStars }, (_, i) => {
          ...
          return { name: `${star} Star${i > 0 ? "s" : ""}`, value: counts[star] || 0 }
        })
        return { type: "bar", questionLabel, data: chartData, totalResponses: responses.length }
      }

      if (["text", "textarea", "email", "number", "date", "time", "color"].includes(field.type)) {
        return { type: "list", questionLabel, data: responses, totalResponses: responses.length }
      }
  ...
  return [
    { type: "line", questionLabel: "Daily Submissions", data: dailyData, totalResponses: submissions.length },
    ...questionCharts,
  ]
}

export default function SummaryPage({ params }: { params: { formId: string } }) {
